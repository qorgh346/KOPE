...
class recon_6face_loss(nn.Module):
    ...
    def cal_recon_loss_vote(self, pc, face_normal, face_dis, face_c, p_rot_g, f_rot_g, p_rot_r, f_rot_r, p_t, p_s,
        ...
        for i in range(bs):
            ...
            n_y_up = torch.nan_to_num(n_y_up)
            dn_y_up = torch.nan_to_num(dn_y_up)
            c_y_up = torch.nan_to_num(c_y_up)
            ...
            if sym[i, 0] == 0:
                ...
                n_x_up = torch.nan_to_num(n_x_up)
                dn_x_up = torch.nan_to_num(dn_x_up)
                c_x_up = torch.nan_to_num(c_x_up)
                # cal gt
                ...
                n_z_up = torch.nan_to_num(n_z_up)
                dn_z_up = torch.nan_to_num(dn_z_up)
                c_z_up = torch.nan_to_num(c_z_up)
                # cal gt
                ...
                n_x_down = torch.nan_to_num(n_x_down)
                dn_x_down = torch.nan_to_num(dn_x_down)
                c_x_down = torch.nan_to_num(c_x_down)
                # cal gt
                dn_gt = -gt_r_x * (-(torch.dot(-gt_r_x, gt_t_now - gt_r_x * re_s_now[0] / 2)))
                # adjust the sign of dn_gt
                ...
                n_z_down = torch.nan_to_num(n_z_down)
                dn_z_down = torch.nan_to_num(dn_z_down)
                c_z_down = torch.nan_to_num(c_z_down)
                # cal gt
                dn_gt = -gt_r_z * (-(torch.dot(-gt_r_z, gt_t_now - gt_r_z * re_s_now[2] / 2)))
                # adjust the sign of dn_gt
            ...
            n_y_down = torch.nan_to_num(n_y_down)
            dn_y_down = torch.nan_to_num(dn_y_down)
            c_y_down = torch.nan_to_num(c_y_down)
            # cal gt
            dn_gt = -gt_r_y * (-(torch.dot(-gt_r_y, gt_t_now - gt_r_y * re_s_now[1] / 2)))
            # adjust the sign of dn_gt
            ...
            # y+
            dis_y_up = torch.abs(torch.dot(torch.nan_to_num(n_y_up), torch.nan_to_num(p_t_now)) + torch.nan_to_num(c_y_up))
            if sym[i, 0] == 0:
                if obj_id != 5:
                    # x+
                    dis_x_up = torch.abs(torch.dot(n_x_up, p_t_now) + c_x_up)
                    # x-
                    dis_x_down = torch.nan_to_num(torch.abs(torch.dot(n_x_down, p_t_now) + c_x_down))
            ...
            if sym[i, 0] == 0:
                if obj_id != 5:
                    res_recon_self_cal += torch.abs(torch.dot(torch.nan_to_num(n_y_up), torch.nan_to_num(n_x_up)))
                    res_recon_self_cal += torch.abs(torch.dot(torch.nan_to_num(n_y_down), torch.nan_to_num(n_x_down)))
                res_recon_self_cal += torch.abs(torch.dot(torch.nan_to_num(n_y_up), torch.nan_to_num(n_z_up)))
                res_recon_self_cal += torch.abs(torch.dot(torch.nan_to_num(n_y_down), torch.nan_to_num(n_z_down)))
