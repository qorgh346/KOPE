...
def crop_roi_mask(mask, crop_size=(256, 256)):
    assert mask.shape == (480, 640), "The input mask should have shape (480, 640)"

    # Find the bounding box of the ROI in the mask
    y, x = np.where(mask > 0)
    ...
    bbox_center = [center_y, center_x]
    scale = max(bbox_height, bbox_width)

    # Crop the mask image
    cropped_mask = mask[y1:y2, x1:x2]

    # Resize the cropped mask to the desired size
    resized_mask = cv2.resize(cropped_mask, crop_size, interpolation=cv2.INTER_NEAREST)

    return resized_mask, bbox_center, float(scale)
