...
class Block(nn.Module):
    ...
    def forward(self, points, reference_index):
        ...
        identity = feat
        #gcn3d
        feat1 = self.act(self.fc1(feat))
        feat1 = self.attn(feat1, coord, reference_index) \
            if not self.enable_checkpoint else checkpoint(self.attn, feat1, coord, reference_index)
        feat1 = self.act(feat1)
        feat2 = self.fc3(feat1)
        feat2 = identity + self.drop_path(feat2)
        feat2 = self.act(feat2)


        return [coord, feat2, offset]
...
class PointTransformerV2Network(nn.Module):
    def __init__(self,
                 in_channels=3,
                 num_classes=13,
                 patch_embed_depth=2,
                 patch_embed_channels=512,
                 patch_embed_groups=8,
                 patch_embed_neighbours=16,
                 enc_depths=(2, 6, 2),
                 enc_channels=(96, 192, 512),
                 enc_groups=(12, 24, 128),
                 enc_neighbours=(16, 16, 16),
                 dec_depths=(1, 1, 1),
                 dec_channels=(512, 96, 192),
                 dec_groups=(8, 16, 24),
        ...
        super(PointTransformerV2Network, self).__init__()
        self.in_channels = in_channels
        ...
        self.global_feator= nn.Sequential(
            nn.Linear(512,1282),
            nn.ReLU(inplace=True)
        )
        ...
        # self.obj_feat_head = nn.Sequential(
        #     nn.Linear(dec_channels[0], 128),
        #     nn.ReLU(),
        #     nn.Linear(128, 256)
        # )

    def mapping_instance(self,obj_num,instance2mask,instances,feature):
        with torch.no_grad():
            instances = instances.squeeze(dim=0).cpu().numpy()
            device = feature.device
            dim_point = feature.size(1)
            num_points = 512
            feature = feature.detach().cpu().numpy()
            masks = np.array(list(map(lambda l: instance2mask[l].cpu().numpy(), instances)), dtype=np.int32)
            obj_points = torch.zeros([obj_num, num_points, dim_point]).to(device)
            for i in range(obj_num):
                obj_pointset = feature[np.where(masks == i + 1)[0], :]
                choice = np.random.choice(len(obj_pointset), num_points, replace=True)
                obj_pointset = obj_pointset[choice, :]
                obj_pointset = torch.from_numpy(obj_pointset.astype(np.float32)).to(device)
                obj_points[i] = obj_pointset
        return obj_points.mean(1)

    def forward(self, obj_num, data_dict):
        coord = data_dict["coord"].squeeze(dim=0)#.to('cuda:0')
        # feat = data_dict["feat"].squeeze(dim=0)#.to('cuda:0')
        offset = data_dict["offset"].squeeze(dim=0).int()
        offset = offset #.to('cuda:0')

        # a batch of point cloud is a list of coord, feat and offset
        points = [coord, coord, offset]
        points = self.patch_embed(points)

        f_global = points[1].detach() #using for gpv-pose
        ...
        coord, feat, offset = points
        global_feat = self.global_feator(feat)
        return global_feat, f_global


if __name__ == '__main__':
    pt_v2 = PointTransformerV2Network().cuda()
    print(pt_v2)
    obj_num = 1

    #Input
    data_dict = {
        'coord':torch.randn(1,1028,3).cuda(),
        'offset':torch.tensor([[1028]]).cuda()
        # 'feat':torch.randn(1,1028,6).cuda()
    }

    x = pt_v2(obj_num,data_dict)
