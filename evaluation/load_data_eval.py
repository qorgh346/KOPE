...
class PoseDataset(data.Dataset):
    def __init__(self, source=None, mode='test',
        ...
        source = 'IsaacSIM'
        # img_list_path = ['IsaacSIM/train_list_all.txt', 'IsaacSIM/test_0425_list_all.txt']
        ...
        self.cat_names = ['box', 'can']
        self.cat_name2id = {'box': 1, 'can': 2}
        self.id2cat_name = {'1': 'box', '2': 'can'}
        ...
        self.real_intrinsics = np.array([[1465.9985736982494, 0, 640.0, ], [0, 1465.9985736982494, 360.0], [0, 0, 1]],
                                        dtype=np.float)
    ...
    def __getitem__(self, index):
        ...
        if os.path.exists(depth_path):
            ...
            return None
        num_instance = len(gts['class_ids']) #len(detection_dict['pred_class_ids'])
        ...
        for j in range(num_instance):
            cat_id = gts['class_ids'][j]#detection_dict['pred_class_ids'][j]
            ...
            if np.sum(depth_valid) <= 1.0:
                print('np.sum(depth_valid) <= 1.0:')
                return None #self.__getitem__((index + 1) % self.__len__()) #None
            ...
            depth_normalize = torch.ones((3,3))#(roi_depth - np.min(depth_v_value)) / (np.max(depth_v_value) - np.min(depth_v_value))
            # depth_normalize[~roi_m_d_valid] = 0.0
            # occupancy canonical
            # sym
            sym_info = self.get_sym_info(self.id2cat_name[str(cat_id+1)])
            mean_shape = self.get_mean_shape(self.id2cat_name[str(cat_id+1)])
            # mean_shape = mean_shape / 1000.0
            mean_shape = mean_shape / 100000.0


            gt_rot = gts['rotations'][j]
            gt_tran = gts['translations'][j]

            gt_rots.append(gt_rot)
            gt_trans.append(gt_tran)
        ...
        roi_depth_norms = torch.ones((1,1))
        ...
        data_dict['gt_rot'] = torch.as_tensor(gt_rots.astype(np.float32)).contiguous()
        data_dict['gt_trans'] = torch.as_tensor(gt_trans.astype(np.float32)).contiguous()
        ...
        return data_dict, detection_dict, gts,img_path
